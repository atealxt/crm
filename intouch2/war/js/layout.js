...
function renderDivs() {
	try {
		...
		YAHOO.claros.calendar.cal1 = new YAHOO.widget.Calendar("cal1","smallCalendar");
		YAHOO.claros.calendar.cal1.render();
	} catch (e) {}
}
		...
		if (operate && curIndex >= 1 && curIndex < ps.length/2) {
			...
			for (var i=0;i<ps.length/2;i++) {
				if (ps[i].clicked) {
					...
					ps[i].getElementsByTagName('span')[0].style.backgroundImage = 'url(images/mini-dot.gif)';
				}
			}
