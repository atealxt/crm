...
function mailListChangeStyle(o,bc,fc,c,lc,bi) {
	...
	if(0==bi) bio.style.backgroundImage='url(images/mini-dot.gif)';
	else bio.style.backgroundImage='url(images/mini-dot-hover.gif)';
}
...
function reLocateFolderActions() {
	try {
		if (!folderActionsExpanded) {
			Dom.get('folderActionsBtn').style.top = Dom.get('folders').scrollHeight + "px"
			Dom.get('folderActions').style.display = 'none';
		} else {
			Dom.get('folderActionsBtn').style.top = (Dom.get('folders').scrollHeight - 143) + "px";
			Dom.get('folderActions').style.top = (Dom.get('folders').scrollHeight - 143) + "px";
			Dom.get('folderActions').style.display = 'block';
		}
	} catch (e) {}
}
