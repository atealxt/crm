		  ...
		  success: 	function(o) {
				if(o.responseText !== undefined) {
					...
					try {
						...
						window.setTimeout(checkNewMailExists, 19000);
					}
					if (tmpUnreadMessages != -1) {
						unreadMessages = tmpUnreadMessages;
					}
					...
					try {
						...
						if (preSelectId != null) {
							...
							if (obj != null) {
								...
								obj.clicked = true;
								obj.getElementsByTagName('span')[0].style.backgroundImage = 'url(images/mini-dot-hover.gif)';
	...
	var callback = {
	  success: 	function(o) {
			if(o.responseText !== undefined) {
				...
				list.innerHTML = o.responseText;
				if(""!=selectedFolder)
					Dom.get('inboxTitle1').innerHTML=Dom.get('mailFolder'+selectedFolder).getAttribute('foldername');
				mailDragFolderInit();
				Dom.get('kitwait').style.display = 'none';
			}
	  },
...
function showMail(id) {
	...
	var paramData = "folder=" + myEncode(folderName) + "&msgId=" + id;
...
//			alert(g.message);
	...
	if (newMsgNo > 0) {
		...
		tmpObj.clicked = true;
		tmpObj.getElementsByTagName('span')[0].style.backgroundImage = 'url(images/mini-dot-hover.gif)';
